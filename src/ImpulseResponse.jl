...
function minimalphase_fir_original(ms::Matrix, mm::Matrix, fs, f0, f1=fs/2, attenuate=-50, unitcircle=1024, tess=30, tdecay=3, noct=3)
    ...
    PyPlot.figure("impulse response and 1/3 oct smoothing")
    ...
    PyPlot.figure("harmonic response")
    PyPlot.semilogx(ω, yd)
    PyPlot.xlabel("Hz")
    PyPlot.grid(true)
...
function minimalphase_fir_design(f, m, ϕ, xds, fs, f0, f1, fx, unitcircle=1024)
        ...
        H[1:ω0-1] .= H[ω0]
        H[ω1+1:end] .= H[ω1]
        PyPlot.figure("minimal-phase compensation filter") 
        PyPlot.semilogx(f, H)
